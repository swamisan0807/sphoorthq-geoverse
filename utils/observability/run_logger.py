...
datasets/reports/runs/ (local), or under "reports/runs/" in STATE_S3_BUCKET
when configured (utils/core/cloud_state.py), so run history survives a
stateless redeploy the same way the model registry does. This is what lets
you answer "what actually happened in this pipeline run, how long did each
stage take, what did it operate on, and what came out" after the fact - the
observability requirement - without bolting on a separate monitoring stack.
"""
...
from utils.core.paths import RUNS_DIR
...
class RunLogger:
    ...
    def finalize(self) -> str:
        ...
        if cloud_state.enabled():
            key = f"reports/runs/{self.run.run_id}.json"
            cloud_state.write_json(key, record)
            out_ref = f"s3://{cloud_state.bucket_name()}/{key}"
        else:
            RUNS_DIR.mkdir(parents=True, exist_ok=True)
            out_path = RUNS_DIR / f"{self.run.run_id}.json"
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(record, f, indent=2)
            out_ref = str(out_path)

        total = round(self.run.ended_at - self.run.started_at, 3)
        print(f"[{self.run.run_name}] run complete in {total}s -> {out_ref}")
        return out_ref
...
def load_recent_runs(limit: int = 20) -> list[dict]:
    ...
    if cloud_state.enabled():
        objects = sorted(
            (o for o in cloud_state.list_objects("reports/runs/") if o["key"].endswith(".json")),
            key=lambda o: o["last_modified"],
            reverse=True,
        )
        return [r for o in objects[:limit] if (r := cloud_state.read_json(o["key"])) is not None]
